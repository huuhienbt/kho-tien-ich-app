(function () {
    ...
    function androidIntentUrl(destination) {
        ...
        const targetPackage = encodeURIComponent(destination.androidPackage);
        return `intent://open-ai?package=${targetPackage}&fallback=${fallback}#Intent;scheme=egv;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;package=vn.egv.app;S.browser_fallback_url=${fallback};end`;
    }
    ...
    function openMobileDestination(destination, operatingSystem) {
        if (operatingSystem === 'android' && destination.androidPackage) {
